...
document.addEventListener("DOMContentLoaded", async () => {
    ...
    // MOD 1 : bouton d'ajout d'un niveau / étage. Le niveau démarre VIDE :
    // le technicien y ajoute lui-même ses points de mesure (chaque point = un
    // lieu avec 4G + 5G).
...
// Pré-traite l'image avant OCR : agrandit (×2), passe en niveaux de gris et
// augmente le contraste. Les écrans Network Cell Info ont un fond NOIR avec des
// chiffres clairs en police "digitale" : on inverse donc pour obtenir du texte
// foncé sur fond clair, ce que Tesseract lit beaucoup mieux. Renvoie un dataUrl.
async function _preprocessForOcr(dataUrl) {
    const img = await new Promise((resolve, reject) => {
        const i = new Image();
        i.onload = () => resolve(i);
        i.onerror = () => reject(new Error("decode"));
    ...
    // Agrandissement (aide la reconnaissance des petites polices / indices Mb/s)
    const scale = (Math.max(w, h) < 1400) ? 2 : 1.4;
    ...
    // Luminance moyenne pour décider d'inverser (fond sombre → inversion)
    let sum = 0;
    for (let i = 0; i < d.length; i += 4) {
        sum += 0.299 * d[i] + 0.587 * d[i + 1] + 0.114 * d[i + 2];
    }
    const avg = sum / (d.length / 4);
    const darkBg = avg < 110;

    // Niveaux de gris + contraste + inversion conditionnelle
    const contrast = 1.6;
    const intercept = 128 * (1 - contrast);
    for (let i = 0; i < d.length; i += 4) {
        let g = 0.299 * d[i] + 0.587 * d[i + 1] + 0.114 * d[i + 2];
        if (darkBg) g = 255 - g;                 // texte clair sur noir → texte foncé sur blanc
        g = g * contrast + intercept;            // accentue le contraste
        g = g < 0 ? 0 : (g > 255 ? 255 : g);
        d[i] = d[i + 1] = d[i + 2] = g;
    }
...
// On essaie d'abord l'image PRÉ-TRAITÉE (meilleure sur les écrans sombres) ;
// si le résultat est trop pauvre, on retente sur l'image d'origine.
async function _ocrText(dataUrl) {
    await ensureTesseract();
    // Paramètres : on autorise chiffres, signes et lettres des libellés.
    const opts = {};
    let best = "";
    try {
        const pre = await _preprocessForOcr(dataUrl);
        const r1 = await window.Tesseract.recognize(pre, "eng", opts);
        best = (r1 && r1.data && r1.data.text) ? r1.data.text : "";
    } catch (e) {
        // ignore : on tentera l'original
    }
    // Si le pré-traitement n'a presque rien donné, retente sur l'original
    const digitsFound = (best.match(/\d/g) || []).length;
    if (digitsFound < 3) {
        try {
            const r2 = await window.Tesseract.recognize(dataUrl, "eng", opts);
            const txt2 = (r2 && r2.data && r2.data.text) ? r2.data.text : "";
            if ((txt2.match(/\d/g) || []).length > digitsFound) best = txt2;
        } catch (e) { /* garde best */ }
    }
    return best;
}

// Découpe une ZONE de l'image (en pourcentages 0..1) et renvoie son dataUrl,
// agrandi et contrasté/inversé pour l'OCR. Utilisé pour lire un seul nombre
// (ex : la valeur centrale d'un cadran) sans le bruit du reste de l'écran.
async function _cropZone(dataUrl, zone, opts = {}) {
    const img = await new Promise((resolve, reject) => {
        const i = new Image();
        i.onload = () => resolve(i);
        i.onerror = () => reject(new Error("decode"));
    ...
    const sx = Math.max(0, Math.round(zone.x * W));
    const sy = Math.max(0, Math.round(zone.y * H));
    const sw = Math.min(W - sx, Math.round(zone.w * W));
    const sh = Math.min(H - sy, Math.round(zone.h * H));
    if (sw <= 0 || sh <= 0) return dataUrl;

    // Agrandissement fort (zone petite → on monte à ~4×) pour la lisibilité.
    const scale = opts.scale || Math.max(2, Math.min(5, 600 / sw));
    ...
    for (let i = 0; i < d.length; i += 4) {
        ...
        g = g < 0 ? 0 : (g > 255 ? 255 : g);
        // binarisation douce pour détacher le chiffre du fond
...
// OCR d'une zone renvoyant le TEXTE BRUT (avant parsing). Permet d'inspecter la
// présence et la position d'un séparateur décimal (essentiel pour le débit :
// on ne pré-remplit que si la virgule est sûre).
async function _ocrRawInZone(dataUrl, zone, whitelist) {
    await ensureTesseract();
    try {
        const cropped = await _cropZone(dataUrl, zone);
        const r = await window.Tesseract.recognize(cropped, "eng", {
...
// les espaces, et la confusion O→0. Reste prudent pour éviter les faux positifs.
...
// Parse STRICT pour le débit : ne renvoie une valeur QUE si une décimale est
// présente ET correctement placée (ex : "92,1", "3.7", "218,9"). Si l'OCR a
// perdu la virgule (ex : "921", "37") ou produit quelque chose d'ambigu, renvoie
// null → le champ reste VIDE (saisie manuelle), conformément au choix : on ne
// remplit jamais un débit dont la virgule n'est pas sûre.
function _parseDebitStrict(raw) {
    if (raw == null) return null;
    // garder uniquement chiffres, séparateurs et signe
    let s = String(raw).replace(/[^\d.,]/g, "");
    // un seul séparateur décimal attendu, suivi d'1 à 2 décimales
    // (les espaces/parasites multiples invalident → on refuse)
...
// Détecte la TECHNO (LTE, 5G, 4G, NR, etc.) à partir du texte OCR plein écran.
// Renvoie une chaîne normalisée à mettre dans le champ "Bande / Techno", ou null.
function extractTechno(text) {
    if (!text) return null;
    const up = text.toUpperCase();
    // ordre de priorité : 5G/NR avant LTE avant 3G
    if (/\b5G\b|\bNR\b|\bN\d{2,3}\b/.test(up)) return "5G";
    if (/\bLTE\b|\b4G\+?\b|\bLTE\+?\b/.test(up)) return up.includes("LTE+") || up.includes("4G+") ? "LTE+ (4G+)" : "LTE (4G)";
    if (/\b3G\b|\bHSPA\b|\bUMTS\b/.test(up)) return "3G";
    return null;
}

// NOTE : pas de lecture automatique de la JAUGE (RSRP/RSRQ/SNR). Tests à l'appui,
// la police "7 segments" de Network Cell Info n'est pas lue de façon fiable par
// Tesseract (segments + segments fantômes de l'afficheur). Pour ne JAMAIS proposer
// une valeur fausse, ces champs restent en saisie manuelle. La copie d'écran reste
// importée et consultable (et agrandissable) à côté des champs.

// Extrait les valeurs de DÉBIT depuis le TEXTE OCR plein écran — mais STRICT :
// uniquement les nombres qui possèdent une décimale fiable (sinon champ laissé vide).
function extractDebitValues(text) {
    const out = {};
    if (!text) return out;

    // On collecte les motifs "<nombre avec décimale> Mb/s" (la décimale est exigée).
    const speeds = [];
    const re = /(\d{1,4}[.,]\d{1,2})\s*(?:mb\s*\/?\s*s|mbps|mbit\/?s|mo\/?s)/gi;
    let m;
    while ((m = re.exec(text)) !== null) {
        const v = _parseDebitStrict(m[1]);
        if (v != null) speeds.push(v);
    }
    if (speeds.length >= 1) out.down = speeds[0];
    if (speeds.length >= 2) out.up   = speeds[1];
    return out;
}

// Extrait les DÉBITS par LECTURE DE ZONES (layout fixe du popup "Résultats des
// tests de vitesse" de Network Cell Info). Deux valeurs côte à côte en bas du popup :
//   - descendant (download) → moitié gauche
//   - montant   (upload)    → moitié droite
//
// SÉCURITÉ MAXIMALE : on lit plusieurs zones/cadrages candidats, et on ne retient
// une valeur QUE si AU MOINS DEUX lectures indépendantes tombent EXACTEMENT sur le
// même nombre (consensus). Une valeur ambiguë (ex : 3,7 lu tantôt "3,1", "9,",
// "27"…) ne fait jamais consensus → le champ reste VIDE (saisie manuelle).
// Ainsi : si c'est sûr on remplit, sinon on ne met rien (jamais de valeur fausse).
async function extractDebitValuesByZone(dataUrl) {
    const out = {};
    const downZones = [
        { x: 0.17, y: 0.615, w: 0.27, h: 0.05 },
        { x: 0.17, y: 0.60,  w: 0.27, h: 0.06 },
        { x: 0.17, y: 0.635, w: 0.18, h: 0.045 },
        { x: 0.18, y: 0.63,  w: 0.26, h: 0.05 }
    ];
    const upZones = [
        { x: 0.52, y: 0.615, w: 0.24, h: 0.05 },
        { x: 0.52, y: 0.60,  w: 0.24, h: 0.06 },
        { x: 0.525, y: 0.64, w: 0.135, h: 0.04 },
        { x: 0.53, y: 0.63,  w: 0.23, h: 0.05 }
    ];

    const readConsensus = async (zones) => {
        const counts = new Map();
        for (const z of zones) {
            const raw = await _ocrRawInZone(dataUrl, z, "0123456789.,");
            const v = _parseDebitStrict(raw);
            if (v == null) continue;
            counts.set(v, (counts.get(v) || 0) + 1);
        }
        // valeur qui apparaît au moins 2 fois (la plus fréquente)
        let best = null, bestN = 0;
        counts.forEach((n, v) => { if (n > bestN) { bestN = n; best = v; } });
        return bestN >= 2 ? best : null;
    };

    const [down, up] = await Promise.all([readConsensus(downZones), readConsensus(upZones)]);
    if (down != null) out.down = down;
    if (up   != null) out.up   = up;
    return out;
}

// Correspondance champ ↔ type de copie d'écran.
//  - "lieu"   (Jauge) : on n'auto-remplit PAS RSRP/RSRQ/SNR (7 segments non fiable),
//                       mais on peut renseigner la TECHNO (LTE/5G) dans "band".
//  - "screen" (Débit) : débit descendant / montant (uniquement si virgule sûre).
const OCR_FIELD_MAP = {
    lieu:   { band: ".measure-band" },
...
async function runOcrForMeasurePhoto(group, photoType, dataUrl) {
    ...
    // Réimport / remplacement : on efface les valeurs OCR précédentes de CETTE
    // photo pour pouvoir les rafraîchir. Les valeurs manuelles ne sont pas touchées.
    clearOcrValuesForPhoto(group, photoType);
    _setOcrStatus(group, photoType, "🔎 Lecture automatique…", "working");
    try {
        let values = {};

        if (isGauge) {
            // ÉCRAN JAUGE : on NE lit PAS RSRP/RSRQ/SNR (police 7 segments non fiable).
            // On tente seulement de récupérer la TECHNO (LTE/5G…) → champ "Bande/Techno".
            const text = await _ocrText(dataUrl);
            const techno = extractTechno(text);
            if (techno) values.band = techno;
        } else {
            // ÉCRAN DÉBIT : lecture par zones (stricte : virgule sûre uniquement),
            // complétée si besoin par le texte plein écran (strict également).
            try { values = await extractDebitValuesByZone(dataUrl); }
            catch (e) { /* fallback texte ci-dessous */ }
            if (values.down == null || values.up == null) {
                const text = await _ocrText(dataUrl);
                const textVals = extractDebitValues(text);
                if (values.down == null && textVals.down != null) values.down = textVals.down;
                if (values.up   == null && textVals.up   != null) values.up   = textVals.up;
            }
        }
        ...
        if (filled > 0) {
            _setOcrStatus(group, photoType,
                `✅ ${filled} valeur${filled > 1 ? "s" : ""} pré-remplie${filled > 1 ? "s" : ""} (vérifiez si besoin).`, "ok");
        } else if (isGauge) {
            _setOcrStatus(group, photoType,
                "ℹ️ Jauge : RSRP / RSRQ / SNR à saisir à la main (lecture auto non fiable sur cet écran).", "warn");
        } else {
            _setOcrStatus(group, photoType,
                "ℹ️ Débit non détecté de façon sûre — saisie manuelle.", "warn");
        }
    } catch (err) {
...
// Helpers de sélection (par CLASSE → agrègent tous les niveaux et tous les lieux).
// IMPORTANT : la classe .mp-container-4g / .mp-container-5g est portée PAR le
// ".measure-point-group" lui-même (sous-bloc d'un lieu), pas par un parent.
function getMeasureContainers(tech) {
    const cls = tech === "5g" ? ".measure-point-group.mp-container-5g" : ".measure-point-group.mp-container-4g";
...
function getAllMeasureGroups(tech) {
    ...
    if (tech === "4g") sel = ".measure-point-group.mp-container-4g";
    else if (tech === "5g") sel = ".measure-point-group.mp-container-5g";
...
function addMeasureLevel(opts = {}) {
    ...
    card.innerHTML = `
        ...
        <div class="mp-locations-container"></div>

        <button type="button" class="btn-secondary add-mp-btn audit-only mp-add-location" style="margin-top:12px;">
            ➕ Ajouter un point de mesure (lieu : 4G + 5G)
        </button>
    `;
    container.appendChild(card);

    // Bouton d'ajout d'un point de mesure (= un lieu avec 4G + 5G), ciblant CE niveau
    card.querySelector(".mp-add-location")?.addEventListener("click", () => addMeasureLocation(card));
...
function initMeasurePoints() {
    ...
    if (getLevelCards().length === 0) {
        // Un niveau de départ, SANS point pré-créé (le technicien ajoute lui-même).
        addMeasureLevel();
    }
}

// ------------------------------------------------------------
//  POINT DE MESURE = UN LIEU contenant la mesure 4G ET la 5G
// ------------------------------------------------------------
// Crée une "carte lieu" dans le niveau donné. Elle contient :
//   - un nom de lieu PARTAGÉ (ex : Aile gauche)
//   - un sous-bloc 4G  (pointId "4g-N", classe .mp-container-4g)
//   - un sous-bloc 5G  (pointId "5g-N", classe .mp-container-5g)
// Les deux sous-blocs restent des ".measure-point-group" autonomes : toute la
// logique existante (qualité, photos, OCR, plans évac, validation, génération
// Word groupée par techno) continue de fonctionner sans modification.
// Le nom de lieu est recopié (caché) dans le ".point-lieu" de chaque sous-bloc.
function addMeasureLocation(levelCard, opts = {}) {
    if (!levelCard) levelCard = getLastLevelCard();
    if (!levelCard) levelCard = addMeasureLevel();
    if (!levelCard) return null;
    const host = levelCard.querySelector(".mp-locations-container");
    if (!host) return null;

    measureCounter4G++;
    measureCounter5G++;
    const n4 = measureCounter4G;
    const n5 = measureCounter5G;
    const pid4 = `4g-${n4}`;
    const pid5 = `5g-${n5}`;

    const locCard = document.createElement("div");
    locCard.className = "mp-location-card";
    locCard.innerHTML = `
        <div class="mp-location-header">
            <span class="mp-location-pin">📍</span>
            <input type="text" class="mp-location-name mp-field" placeholder="Lieu / Pièce (ex : Aile gauche, Hall d'entrée, Aile droite...)" value="${escapeHtml(opts.lieu || "")}">
            <button type="button" class="btn-delete mp-location-remove" title="Supprimer ce point de mesure">🗑</button>
        </div>
        <div class="mp-location-techs"></div>
    `;
    host.appendChild(locCard);

    const techsWrap = locCard.querySelector(".mp-location-techs");
    const sub4 = buildMeasureSubBlock("4g", pid4, n4);
    const sub5 = buildMeasureSubBlock("5g", pid5, n5);
    techsWrap.appendChild(sub4);
    techsWrap.appendChild(sub5);

    // Nom de lieu PARTAGÉ → synchronisé dans le ".point-lieu" (caché) de chaque sous-bloc
    const nameInput = locCard.querySelector(".mp-location-name");
    const syncLieu = () => {
        const v = nameInput.value;
        [sub4, sub5].forEach(sb => {
            const hidden = sb.querySelector(".point-lieu");
            if (hidden) hidden.value = v;
        });
        updateEvacPointLabel(pid4);
        updateEvacPointLabel(pid5);
        refreshEvacPicker();
        if (window.__autosaveSchedule) window.__autosaveSchedule();
    };
    nameInput.addEventListener("input", syncLieu);
    syncLieu(); // initialise (cas restauration avec opts.lieu)

    // Suppression du LIEU = suppression des 2 sous-blocs (4G + 5G)
    locCard.querySelector(".mp-location-remove")?.addEventListener("click", () => {
        const hasContent = [pid4, pid5].some(pid =>
            photoStore[`mesure_lieu_${pid}`] || photoStore[`mesure_screen_${pid}`]) ||
            (nameInput.value || "").trim() ||
            [sub4, sub5].some(sb => ["rsrp","rsrq","sinr","down","up","band"].some(c =>
                (sb.querySelector(".measure-"+c)?.value || "").trim()));
        if (hasContent && !confirm("Supprimer ce point de mesure (mesures 4G + 5G de ce lieu) ?")) return;
        [pid4, pid5].forEach(pid => {
            delete photoStore[`mesure_lieu_${pid}`];
            delete photoStore[`mesure_screen_${pid}`];
            removeMeasurePointFromAllPlans(pid);
        });
    ...
    if (typeof updateGenerateButtonState === "function") updateGenerateButtonState();
    if (window.__autosaveSchedule) window.__autosaveSchedule();
    return { locCard, sub4, sub5, pid4, pid5 };
}

// Construit UN sous-bloc de mesure (4G ou 5G) : c'est un ".measure-point-group"
// complet (photos, champs, qualité, OCR) MAIS sans champ "lieu" visible (le lieu
// est géré au niveau de la carte lieu et recopié dans un ".point-lieu" caché).
function buildMeasureSubBlock(tech, pid, count) {
    const techLabel = tech === "4g" ? "4G" : "5G";
    const div = document.createElement("div");
    div.className = "measure-point-group " + (tech === "4g" ? "mp-container-4g" : "mp-container-5g");
    div.dataset.point = count;
    div.dataset.tech = tech;
    div.dataset.pointId = pid;
    div.innerHTML = `
        <div class="mp-subhead" data-tech="${tech}">
            <span class="mp-subhead-badge">${tech === "4g" ? "📡 4G" : "🛰️ 5G"}</span>
        </div>
        <input type="hidden" class="point-lieu">
...
// Compatibilité : addMeasurePoint(tech, levelCard) est encore appelé par
// applyTravauxLayout() et l'ancien chemin de restauration. On le redirige vers
// le nouveau modèle "lieu" : ajouter un point 4G OU 5G revient à ajouter un lieu
// complet (4G + 5G). Renvoie le sous-bloc de la techno demandée pour les
// appelants qui veulent le remplir.
function addMeasurePoint(tech, levelCard) {
    if (tech !== "4g" && tech !== "5g") tech = "4g";
    const res = addMeasureLocation(levelCard);
    if (!res) return null;
    return tech === "5g" ? res.sub5 : res.sub4;
}
...
const groups4G = Array.from(document.querySelectorAll('.measure-point-group.mp-container-4g')).filter(_isVisible);
...
// ---------- 3.B — 5G ----------
const groups5G = Array.from(document.querySelectorAll('.measure-point-group.mp-container-5g')).filter(_isVisible);
...
if (_modeS3 === "travaux" && photoStore['routeur_install']) {
   ...
   function photoBannerInner(title, key, maxWidthTwips) {
                            ...
                            children: [
                                ...
                                new TableCell({
                                    ...
                                    children: [new Paragraph({
                                        ...
                                        children: (clientLogoBytes() ? [new ImageRun({
                                            data: clientLogoBytes(),
                                            transformation: clientLogoSize(60)
                                        })] : [])
                                    })]
                                })
                            ]
    ...
    // ⚠️ FIX ENVOI SERVEUR : il faut ATTENDRE la génération du blob.
    // Avant, Packer.toBlob(...).then(...) n'était pas attendu →
    // captureSaveAs() (auth-upload) se terminait AVANT l'appel à saveAs()
    // et l'envoi échouait toujours avec « Aucun fichier généré ».
...
const AUTOSAVE = (() => {
    ...
    // Estime si un jeu de données est "essentiellement vide" (aucune saisie
    // utile). Sert de GARDE-FOU : on ne remplace JAMAIS une sauvegarde
    // remplie par un formulaire vide — c'était la cause des « formulaires
    // vides » : si la restauration échouait/tardait, le 1er évènement input
    // écrasait la bonne sauvegarde avec un formulaire vierge.
    function _isEmptyData(d) {
        ...
        const hasPoint = Array.isArray(d.measurePoints) && d.measurePoints.length > 0 &&
                         d.measurePoints.some(p => (p.lieu || p.rsrp || p.sinr || p.down));
        return !hasField && !hasPhoto && !hasPoint;
    }
    ...
    async function _saveNow() {
        ...
        try {
            ...
            // Garde-fou anti-écrasement : formulaire vide + sauvegarde pleine → on ne touche à rien
    ...
    async function restore() {
        ...
        try {
            ...
            // On laisse un délai pour que les chargements d'images / setState
            // asynchrones se terminent avant de réactiver la sauvegarde.
            // (1500 ms : les photos base64 + plans peuvent être longs à
            // ré-appliquer sur mobile — 400 ms étaient parfois trop courts et
            // un autosave partiel écrasait la bonne sauvegarde.)
    ...
    function attach() {
        ...
        // Filets de sécurité — IMPORTANT pour mobile/tablette :
        // sur Android/iOS, "beforeunload" n'est souvent PAS déclenché quand
        // l'onglet est tué ou la page rechargée. "pagehide" et
        // "visibilitychange" (état hidden) sont les évènements fiables.
        ...
        // Sauvegarde périodique de sécurité : couvre les mutations
        // programmatiques (annotations, points sur plan...) qui ne
        // déclenchent pas d'évènement input/change.
...
function collectFormData() {
    ...
    data.measureLevels = getLevelCards().map(card => ({
        ...
        points4g: Array.from(card.querySelectorAll('.measure-point-group.mp-container-4g'))
                    .map(g => g.dataset.pointId),
        points5g: Array.from(card.querySelectorAll('.measure-point-group.mp-container-5g'))
                    .map(g => g.dataset.pointId)
    }));
...
async function applyFormData(data) {
    ...
    // Client final : Bouygues par défaut pour les anciens JSON (rétrocompatibilité)
    const selClient = document.getElementById("client_final");
    if (selClient && (!data.fields || !data.fields.client_final)) selClient.value = "bouygues";
    ...
    if (levelsContainer && data.measurePoints) {
        ...
        // Remplit un sous-bloc DOM (4G ou 5G) à partir d'un enregistrement point.
        ...
        // Crée UN lieu (4G + 5G) sur un niveau et remplit les deux sous-blocs.
        // mp4 / mp5 : enregistrements des points 4G / 5G (l'un des deux peut manquer).
        const addLocationAndFill = (card, mp4, mp5) => {
            const lieu = (mp4 && mp4.lieu) || (mp5 && mp5.lieu) || "";
            const res = addMeasureLocation(card, { lieu });
            if (!res) return;
            if (mp4) fillGroup(res.sub4, mp4);
            if (mp5) fillGroup(res.sub5, mp5);
        };

        const levels = Array.isArray(data.measureLevels) ? data.measureLevels : null;

        if (levels && levels.length > 0) {
            // ----- Format à niveaux : on apparie points4g[i] ↔ points5g[i] en lieux -----
            levels.forEach(lvl => {
                const card = addMeasureLevel({ title: lvl.title });
                const p4 = lvl.points4g || [];
                const p5 = lvl.points5g || [];
                const n = Math.max(p4.length, p5.length);
                for (let i = 0; i < n; i++) {
                    addLocationAndFill(card, byPid[p4[i]], byPid[p5[i]]);
                }
            });
            // Points orphelins (référencés par aucun niveau) → 1er niveau, en lieux
            const placed = new Set();
            levels.forEach(l => { (l.points4g||[]).forEach(p=>placed.add(p)); (l.points5g||[]).forEach(p=>placed.add(p)); });
            const orphans4 = data.measurePoints.filter(mp => mp.pointId && mp.tech !== "5g" && !placed.has(mp.pointId));
            const orphans5 = data.measurePoints.filter(mp => mp.pointId && mp.tech === "5g" && !placed.has(mp.pointId));
            if (orphans4.length || orphans5.length) {
                const card = getLevelCards()[0] || addMeasureLevel();
                const n = Math.max(orphans4.length, orphans5.length);
                for (let i = 0; i < n; i++) addLocationAndFill(card, orphans4[i], orphans5[i]);
            }
        } else if (data.measurePoints.length > 0) {
            // ----- Ancien format (sans niveaux) : on apparie 4G et 5G dans l'ordre,
            //       en lieux, sur un seul niveau. -----
            const card = addMeasureLevel();
            const list4 = data.measurePoints.filter(mp => (isLegacy ? "4g" : (mp.tech === "5g" ? "5g" : "4g")) === "4g");
            const list5 = data.measurePoints.filter(mp => (mp.tech === "5g"));
            const n = Math.max(list4.length, list5.length);
            for (let i = 0; i < n; i++) addLocationAndFill(card, list4[i], list5[i]);
        }

        // Restaurer le compteur de niveaux si fourni
        ...
        // Si l'export ne contenait aucun point, on remet un niveau vide par défaut
