...
// Les écrans de test affichent souvent DEUX libellés "Télécharger" (l'un pour le
// descendant, l'autre pour le montant) avec la valeur en "Mb/s" juste en dessous.
// On capte donc, dans l'ordre : les valeurs explicitement marquées download/upload,
// puis à défaut les nombres associés aux libellés "Télécharger/Téléverser", puis
// à défaut les nombres suivis de "Mb/s".
function extractDebitValues(text) {
    const out = {};

    // 1) mots-clés explicites descendant / montant
    const down = _findValueNear(text, ["download", "descendant", "↓", "down", "téléchargement", "telechargement"]);
    const up   = _findValueNear(text, ["upload", "montant", "↑", "up", "téléversement", "televersement", "envoi"]);
    if (down != null && down >= 0 && down < 100000) out.down = down;
    if (up   != null && up   >= 0 && up   < 100000) out.up   = up;

    // 2) Fallback "Mb/s" : tous les nombres suivis d'une unité de débit, dans l'ordre.
    //    Tolère "Mb/s", "Mbps", "Mbit/s", "Mo/s", et un éventuel espace/retour ligne.
    const speeds = [];
    const re = /(-?\d+(?:[.,]\d+)?)\s*(?:mb\s*\/?\s*s|mbps|mbit\/?s|mo\/?s)/gi;
    let m;
    ...
    // 3) Fallback "Télécharger <nombre>" (le libellé peut être identique pour les
    //    deux sens) : on récupère les nombres apparaissant près de "télécharger".
    const teleVals = [];
    if (speeds.length < 2) {
        const lines = text.split(/\n+/).map(l => l.trim());
        for (let i = 0; i < lines.length; i++) {
            if (/t[ée]l[ée]charger|t[ée]l[ée]verser/i.test(lines[i])) {
                const here = (lines[i].match(/-?\d+(?:[.,]\d+)?/g) || []).map(_parseNum).filter(x => x != null);
                const next = ((lines[i + 1] || "").match(/-?\d+(?:[.,]\d+)?/g) || []).map(_parseNum).filter(x => x != null);
                [...here, ...next].forEach(v => { if (v >= 0 && v < 100000) teleVals.push(v); });
            }
        }
    }

    const pool = speeds.length ? speeds : teleVals;
    if (out.down == null && pool.length >= 1) out.down = pool[0];
    if (out.up   == null && pool.length >= 2) out.up   = pool[1];
