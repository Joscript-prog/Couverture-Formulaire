...
function applyLoggedInState(user) {
    ...
    // Vérifier s'il y a un JSON à charger automatiquement
    // (envoyé depuis le dashboard admin via "Ouvrir dans le formulaire")
    setTimeout(tryAutoImportJson, 800);
}

// =================================================================
//  AUTO-IMPORT JSON depuis le dashboard admin
//  Si l'admin a cliqué sur ✏️ "Ouvrir dans le formulaire", il a
//  stocké le JSON dans localStorage. On le récupère et on l'injecte
//  dans le formulaire via la fonction d'import existante.
// =================================================================
function tryAutoImportJson() {
    const raw = localStorage.getItem("__techcouv_import_pending__");
    if (!raw) return;
    ...
    try {
        ...
        localStorage.removeItem("__techcouv_import_pending__");
        return;
    }

    // Sécurité : on ne charge que si le JSON est récent (< 2 minutes)
    if (!payload.timestamp || Date.now() - payload.timestamp > 2 * 60 * 1000) {
        localStorage.removeItem("__techcouv_import_pending__");
        return;
    }

    // On retire immédiatement la clé pour qu'elle ne se réutilise
    // pas si l'utilisateur recharge la page.
    localStorage.removeItem("__techcouv_import_pending__");

    // Convertir la chaîne JSON en File (pour réutiliser les fonctions
    // d'import existantes des formulaires).
    const blob = new Blob([payload.jsonText], { type: "application/json" });
    const file = new File([blob], payload.fileName + ".json", { type: "application/json" });

    // Appeler la bonne fonction selon le formulaire courant
    const dossier = window.__DOSSIER;
    try {
        if (dossier === "pico-quatra" && typeof window.importFromJSON === "function") {
            window.importFromJSON(file);
        } else if (dossier === "starlink" && typeof window.importFromJSON === "function") {
            window.importFromJSON(file);
        ...
        showAutoImportBanner(payload.fileName);
    } catch (err) {
        console.error("Erreur d'auto-import :", err);
    }
}

// Affiche un bandeau d'info au technicien pour qu'il sache que
// le formulaire a été pré-rempli par l'admin.
function showAutoImportBanner(fileName) {
    // On évite de dupliquer le bandeau
    if (document.getElementById("__autoImportBanner__")) return;

    const banner = document.createElement("div");
    banner.id = "__autoImportBanner__";
    ...
    `;
    banner.innerHTML = `
        ✏️ <span>Formulaire pré-rempli depuis le dashboard admin : <strong>${escapeHtmlSafe(fileName)}</strong></span>
        <button id="__closeBanner__" style="background:rgba(255,255,255,0.25); color:#fff; border:none; padding:6px 14px; border-radius:4px; cursor:pointer; font-weight:600;">Fermer</button>
    `;
    document.body.appendChild(banner);
    document.getElementById("__closeBanner__").addEventListener("click", () => banner.remove());
...
function buildServerFilename() {
    ...
    // ⚠️ Supabase Storage REFUSE les clés contenant des caractères non-ASCII
    // (accents : é, è, à...) ou certains caractères spéciaux → erreur
    // « Invalid key » et l'envoi échouait pour les villes/raisons sociales
    // accentuées (ex : "Saint-Étienne", "Société Générale").
    const clean = (s) => String(s)
        .normalize("NFD").replace(/[\u0300-\u036f]/g, "")  // é→e, à→a, ç→c...
        .replace(/[\/\\:*?"<>|#%&{}$!'@+`=~^\[\]]/g, "")   // caractères interdits/risqués
        .replace(/[^\x20-\x7E]/g, "")                      // tout non-ASCII restant
        .replace(/\s+/g, " ")
        .trim();
